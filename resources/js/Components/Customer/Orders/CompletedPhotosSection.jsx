...
export default function CompletedPhotosSection({ order }) {
    const [selectedPhoto, setSelectedPhoto] = useState(null);

    if (!order.completed_photo_path) return null;
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {Array.isArray(order.completed_photo_path) ? (
                    order.completed_photo_path.map((photo, index) => (
                        ...
                        >
                            <img
                                src={`/storage/${photo}`}
                                alt={`Completed ${index}`}
                                className="w-full h-48 md:h-64 object-cover rounded-md border shadow-sm transition-transform duration-300 group-hover:scale-105"
                            />
                    ...
                    <div
                        className="relative group cursor-pointer"
                        onClick={(e) =>
                            openPhoto(order.completed_photo_path, e)
                        }
                    >
                        <img
